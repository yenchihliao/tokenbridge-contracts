...
async function main() {
  ...
  const bridge = await ethers.getContractAt("ForeignAMB", config.bridge);
  ...
  // transfer ownership
  newValidator = "0x...";
  tx = await validator.transferOwnership(newValidator); rc = await tx.wait(); console.log(rc.status);
